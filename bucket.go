...
// 添加JobId到bucket中
func pushToBucket(key string, timestamp int64, jobNo string) error {
	_, err := execRedisCommand("ZADD", key, timestamp, jobNo)
...
func getFromBucket(key string) (*BucketItem, error) {
	...
	item.jobNo = string(valueBytes[0].([]byte))

	return item, nil
}

// 从bucket中删除JobId
func removeFromBucket(bucket string, JobNo string) error {
	_, err := execRedisCommand("ZREM", bucket, JobNo)
